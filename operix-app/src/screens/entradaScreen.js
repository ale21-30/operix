...
export default function EntradaScreen({ navigation }) {
  ...
  // Al abrir la pantalla solicita permiso y obtiene ubicación
  useEffect(() => {
    obtenerUbicacion();
  }, []);
  ...
  const obtenerUbicacion = async () => {
    ...
    try {
      ...
      if (status !== 'granted') {
        setErrorGPS('Permiso de ubicación denegado. Por favor actívalo en Configuración.');
      ...
      setErrorGPS('No se pudo obtener la ubicación. Verifica que el GPS esté activado.');
    } finally {
      setCargandoGPS(false);
    }
  };
  ...
  const handleEntrada = async () => {
    ...
    try {
      ...
      Alert.alert(
        '✅ Entrada registrada',
        `Hora: ${respuesta.hora}\nSede: ${respuesta.sede}\nDistancia a sede: ${respuesta.distancia}m`,
    ...
    <ScrollView contentContainerStyle={styles.container}>
      <View style={styles.card}>
        ...
        <Text style={styles.descripcion}>
          Al registrar tu entrada se guardará la hora actual y tu ubicación GPS como evidencia de inicio de turno.
        </Text>
          ...
          {cargandoGPS ? (
            ...
            <View>
              ...
              <Text style={styles.gpsPrecision}>
                Precisión: ±{ubicacion.precision}m
              </Text>
            </View>
          ) : null}
        </View>
